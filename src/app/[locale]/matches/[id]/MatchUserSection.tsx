    ...
    return (
      <div className="bg-[#0F2040] border border-[#1E3A5F] rounded-2xl p-5 text-center mb-4">
        ...
        <p className="text-gray-400 text-sm">
          {lc(locale, "比赛已结束，预测已关闭。", "This match has ended. Predictions are closed.")}
        </p>
        {bet && !loading && (
          <div className={`mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold ${
            ...
            {bet.status === "won"  ? (lc(locale, "🎉 恭喜获奖！", "🎉 You won!")) :
             bet.status === "lost" ? (lc(locale, "😔 下次加油", "😔 Better luck")) :
             (lc(locale, "⏳ 等待结算", "⏳ Pending"))}
          </div>
        )}
      </div>
    );
  ...
  if (!userId) {
    return (
      <div className="bg-[#0F2040] border border-[#1E3A5F] rounded-2xl p-6 text-center mb-4">
        ...
        <p className="text-white font-bold mb-1">{lc(locale, "登录即可参与预测", "Login to Predict")}</p>
        <p className="text-gray-500 text-sm mb-5">
          {lc(locale, "注册账号，参与预测，赢取GoalCoin！", "Sign in to place your prediction and earn GoalCoins")}
        </p>
