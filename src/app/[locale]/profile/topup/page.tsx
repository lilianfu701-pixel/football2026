  ...
  async function createPayPalOrder() {
    ...
    try {
      ...
      if (!res.ok || !data.orderID) {
        ...
        setPayErr(zh ? `创建订单失败：${msg}` : locale === "es" ? `Error al crear el pedido: ${msg}` : locale === "fr" ? `Échec de la création : ${msg}` : locale === "de" ? `Bestellung fehlgeschlagen: ${msg}` : `Create order failed: ${msg}`);
        throw new Error(msg);
      }
      ...
      if (!payErr) {
        ...
        setPayErr(zh ? `网络错误：${msg}` : locale === "es" ? `Error de red: ${msg}` : locale === "fr" ? `Erreur réseau : ${msg}` : locale === "de" ? `Netzwerkfehler: ${msg}` : `Network error: ${msg}`);
      }
      throw err;
    }
  }
