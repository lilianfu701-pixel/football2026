...
│   │                           #    处理：mobile UA 重定向、Supabase session 刷新、intl 路由
│   ├── app/
...
│   │   │   ├── mobileAuth.ts   # 含 MOBILE_OAUTH_NEXT_COOKIE 常量（proxy.ts 也引用）
│   │   │   └── ...
...
```
704245b fix: repair desktop login/logout auth flow and header sync  ← 最新
...
### 未提交的工作
- `src/components/ProfileCompletion.tsx` — i18n 修改（zh → lc()）尚未提交
- `src/components/mobile/**` — 用户持续开发中，多处未提交
...
*此文件由 Claude Code 自动生成，反映截至 2026-06-08 的项目真实状态。*
